...
map("i", "<C-BS>", "<C-w>", "Apagar palavra anterior (estilo Windows)")
map("i", "<C-?>", "<C-w>", "Apagar palavra anterior (fallback)")
map("c", "<C-BS>", "<C-w>", "Apagar palavra anterior no cmdline")
map("c", "<C-?>", "<C-w>", "Apagar palavra anterior no cmdline (fallback)")
...
map({ "n", "v" }, "<M-Down>", "<Plug>(VM-Add-Cursor-Down)", "Multi-cursor: adicionar cursor abaixo", { noremap = true })
map({ "n", "v" }, "<M-Up>", "<Plug>(VM-Add-Cursor-Up)", "Multi-cursor: adicionar cursor acima", { noremap = true })
...
map("n", "<C-S-O>", "<cmd>Telescope lsp_document_symbols<CR>", "Ir para símbolo no ficheiro")
map("n", "<leader>so", "<cmd>Telescope lsp_document_symbols<CR>", "Ir para símbolo no ficheiro")
map("n", "<leader>nh", "<cmd>nohlsearch<CR>", "Limpar highlight da pesquisa")
map("n", "<Esc><Esc>", "<cmd>nohlsearch<CR>", "Limpar highlight da pesquisa")
...
map("n", "<leader>xw", "<cmd>Trouble diagnostics toggle<CR>", "Diagnostics workspace")
map("n", "<leader>xr", "<cmd>Trouble lsp_references toggle<CR>", "Referências em painel")
map("n", "<leader>xi", "<cmd>Trouble lsp_implementations toggle<CR>", "Implementações em painel")
map("n", "<leader>xd", "<cmd>Trouble lsp_definitions toggle<CR>", "Definições em painel")
...
map("n", "<leader>or", function()
  require("config.tasks").run_template_picker()
end, "Templates de task do projeto")
map("n", "<leader>oR", "<cmd>OverseerRun<CR>", "Correr task manual")
map("n", "<leader>ot", "<cmd>OverseerToggle<CR>", "Toggle painel de tasks")
map("n", "<leader>oo", "<cmd>OverseerOpen<CR>", "Abrir painel de tasks")
map("n", "<leader>oa", "<cmd>OverseerTaskAction<CR>", "Ação na task")
map("n", "<leader>ob", function() run_project_task("build") end, "Build do projeto")
map("n", "<leader>op", function() run_project_task("run") end, "Run do projeto")
map("n", "<leader>oy", function() run_project_task("test") end, "Test do projeto")
map("n", "<leader>go", "<cmd>DiffviewOpen<CR>", "Abrir diffview")
map("n", "<leader>gc", "<cmd>DiffviewClose<CR>", "Fechar diffview")
map("n", "<leader>gh", function() require("gitsigns").preview_hunk() end, "Preview hunk")
map("n", "<leader>gb", function() require("gitsigns").blame_line({ full = true }) end, "Git blame linha")
map("n", "<leader>gs", function() require("gitsigns").stage_hunk() end, "Stage hunk")
map("n", "<leader>grh", function() require("gitsigns").reset_hunk() end, "Reset hunk")
...
map("n", "<F12>", vim.lsp.buf.definition, "Ir para definição (VSCode)")
map("n", "<S-F12>", vim.lsp.buf.references, "Mostrar referências (VSCode)")
map("n", "<C-F12>", vim.lsp.buf.implementation, "Ir para implementação (VSCode)")
map("n", "<M-F12>", preview_definition_or_hover, "Preview definição (VSCode)")
...
map("n", "<leader>ss", function() require("persistence").load() end, "Restaurar sessão do projeto")
map("n", "<leader>sl", function() require("persistence").load({ last = true }) end, "Restaurar última sessão")
map("n", "<leader>sd", function() require("persistence").stop() end, "Desligar persistência de sessão")

-- Debugger (DAP)
map("n", "<F5>", function() require("dap").continue() end, "Debug continue")
map("n", "<S-F5>", function() require("dap").terminate() end, "Debug stop")
...
map("n", "<S-F11>", function() require("dap").step_out() end, "Debug step out")
